...
#define PEAK 255
#define START_PIN 0
#define LOWEST_FREQ 0.1
#define CYCLES_PER_PIO_LOOP 68
...
void main () {
	...
	pio = pio0;
	uint offset = pio_add_program(pio, &eightbit_r2r_dac_lfo_simple_program);
	sm = pio_claim_unused_sm(pio, true);
	
	float init_clkdiv = calc_clkdiv(0.2, WAVESIZE);
	
	eightbit_r2r_dac_lfo_simple_init(pio, sm, offset, START_PIN, init_clkdiv);
